...
const preparationAddCategory = () => {
  const openAddCategoryModalBtn = document.querySelector(
    ".open-add-category-modal-btn",
  );
  const closeAddCategoryModalBtn = document.querySelector(
    ".close-add-category-modal-btn",
  );
...
const addCategory = async (name) => {
  if (name.length <= 3) {
    ...
    _changeClasses("remove", document.querySelector("#add-category-modal"), [
      "show",
    ]);
    _changeClasses("remove", overlay, ["show"]);
